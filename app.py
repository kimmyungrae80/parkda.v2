...
# 3. ✨ 세련된 파스텔 UI 스타일링 (올려주신 이미지 색감 반영)
st.markdown("""
    <style>
    @import url('https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.css');
    
    /* 전체 배경: 깨끗하고 밝은 그레이시 화이트 */
    ...
    /* 대형 메뉴 버튼 (Picton Blue, Java, Danube 컬러 적용) */
    .stButton>button {
        width: 100%; height: 90px !important; font-size: 24px !important; font-weight: 700 !important;
    ...
    .news-link { margin-bottom: 12px; border-bottom: 1px solid #E2E8F0; padding-bottom: 8px; }
    .news-link a { color: #475569 !important; text-decoration: none; font-size: 15px; font-weight: 500; }

    /* 콘텐츠 카드 (Snuff & Powder Ash 스타일 응용) */
    .content-card {
    ...
    /* 조 편성 결과 박스 */
    .team-box { 
        padding: 18px; background-color: #F8FAFC; 
        border-radius: 12px; border: 1px solid #CBD5E1; 
        margin-bottom: 10px; font-size: 22px !important; color: #1E293B; font-weight: bold;
    }

    /* 가이드 텍스트 및 입력창 */
    .guide-text { color: #64748B; font-size: 16px; margin-bottom: 10px; }
    input, textarea { background-color: #FFFFFF !important; border: 1px solid #E2E8F0 !important; font-size: 18px !important; }
    </style>
...
# 5. 사이드바 (로고 상단 고정 & 뉴스 스크롤)
with st.sidebar:
    # 로그인 전/후 모두 로고 상단 배치
    if logo_wide and not st.session_state.logged_in:
        st.image(logo_wide, use_container_width=True)
    elif logo_sq and st.session_state.logged_in:
    ...
    for n in get_clean_news("파크골프")[:20]:
...
if st.session_state.logged_in:
    ...
    if st.session_state.menu_select == "대회정보":
        ...
        for e in get_clean_news("파크골프 대회 공고")[:12]:
            st.markdown(f"<div class='content-card'><a href='{e.link}' target='_blank' style='color:#1E293B; font-size:20px; font-weight:700; text-decoration:none;'>🏆 {e.title}</a><br><small style='color:#64748B;'>{e.published[:16]}</small></div>", unsafe_allow_html=True)

    elif st.session_state.menu_select == "전국지도":
        st.markdown("<p class='guide-text'>💡 깃발을 클릭하면 실시간 날씨 정보를 볼 수 있습니다.</p>", unsafe_allow_html=True)
        ...
        if df_map is not None:
            ...
            for _, row in df_map.iterrows():
                try:
                    ...
                    p_html = f"<div style='font-size:16px; color:#1E293B;'><b>{row[col_map['name']]}</b><hr>{weather}</div>"
                    folium.Marker([lat, lon], popup=folium.Popup(p_html, max_width=250), icon=folium.Icon(color='cadetblue', icon='flag')).add_to(m)
                except: continue
            folium_static(m, width=1000, height=550)

    elif st.session_state.menu_select == "조편성":
        st.markdown("<p class='guide-text'>💡 명단을 붙여넣으세요. 엑셀이나 한글 명단도 자동으로 인식합니다.</p>", unsafe_allow_html=True)
        raw = st.text_area("명단 입력", height=150, placeholder="홍길동 김철수 박지성...")
    ...
    st.info("🔒 회원 인증 후 이용이 가능합니다. 왼쪽 사이드바에서 성함과 연락처를 입력해주세요.")